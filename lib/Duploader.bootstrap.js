...
DuploaderBootstrap.prototype.on_error = function(info){
	$("#upload_err").text(info).show();
}
...
DuploaderBootstrap.prototype.build_modal = function(id){
	...
	var modal_error = $("<h5>").addClass("modal-title text-danger").attr('id','upload_err').css({'display':'none'}).text('');
	...
	var button_upload = $("<button>").addClass("btn btn-info").attr('type','button').css({'margin-left':'10px'}).attr('id','file_upload_' + id).text('上传');
	modal_header.append(button_close).append(modal_title).append(modal_error);
