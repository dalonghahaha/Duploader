...
DuploaderBootstrap.prototype.on_file_add = function(uploader,file_info){
    ...
    if(uploader.config.multiple){
    	...
    	if(span){
    		...
    		uploader.file_changed(file_info);
    	} else{
...
DuploaderBootstrap.prototype.on_file_uploading = function(file_info){
...
DuploaderBootstrap.prototype.on_file_list_upload_finish = function(){
	this.modal.modal('hide');
	if(this.uploader.config.multiple){
		this.success_callback(this.uploader.runtime.file_list);
	}else{
		this.success_callback(this.uploader.runtime.file_list[0]);
	}
}
