...
Duploader.prototype.init = function(){
    ...
    if(this.config.accept_mime){
        selector.attr("accept",this.config.accept_mime);
    }
...
Duploader.prototype.file_selected = function(event) {
    ...
    //自定义校验
    if(this.config.on_file_check && this.config.on_file_check instanceof Function){
        if (!this.config.on_file_check(file_info)){
            return false;
        }
    }
    if (this.runtime.selector.attr("file_id") != 0) {
        if (file_info) {
            ...
            file_info.begin_index = get_file_broken_point(file_info);
...
Duploader.prototype.file_list_finish = function() {
    ...
    if (this.config.on_file_list_upload_finish && this.config.on_file_list_upload_finish instanceof Function) {
        if(this.config.multiple){
            this.config.on_file_list_upload_finish(this.runtime.upload_result);
        } else {
            this.config.on_file_list_upload_finish(this.runtime.upload_result[0]);
        }
    } else {
        this.alert('上传完毕');
    }
